    ...
    <>
      <div className=" flex flex-col justify-start mb-20">
        ...
        <div className="flex items-center mt-10 justify-between mx-40">
          ...
          <div className="flex items-center gap-10">
            ...
            <div className="flex flex-col text-grey text-xl justify-start gap-2 items-start">
              ...
              <p className="">{users.year} Year <span> &#8226; </span> {users.admNumber}</p>
          ...
          <div className="flex items-center gap-10">
            ...
            <div className="w-72">
                <div className="">
                  {validator.isURL(users.resume) ? (<img className=" border-2 border-light-blue rounded-t-lg h-32 w-72 object-cover" src={users.resume} alt="" />)
                  : <div className="flex h-30 border-2 border-light-blue rounded-t-xl w-72 justify-center items-center flex-col text-red text-xl py-2"><Warning/>No resume uploaded</div>}
                </div>
        ...
        <div className="flex flex-col mt-16 justify-center mx-96">
          <p className="mx-auto mb-5 font-bold text-3xl text-grey">Submitted Links</p>
          <div className="grid grid-rows-3 gap-6 grid-cols-2">

            <div className={`profile_links flex items-center px-4 py-3 rounded-full justify-between text-2xl text-white ${!users.links.github ? 'bg-blue/30' : 'bg-blue hover:bg-blue/80'}`}>
              <Github/>
              {users.links.github ? <a target="_blank" rel="noreferrer" className="overflow-hidden line-clamp-1 text-ellipsis underline" href={users.links.github}>{users.links.github}</a>
              : <p className="text-2xl text-grey">Not Submitted</p>}
              <Extlink />
            </div>

            <div className={`profile_links flex items-center px-4 py-3 rounded-full justify-between text-2xl text-white ${!users.links.linkedin ? 'bg-blue/30' : 'bg-blue hover:bg-blue/80'}`}>
              <Linkedin/>
              {users.links.linkedin ? <a target="_blank" rel="noreferrer" className="overflow-hidden line-clamp-1 text-ellipsis underline" href={users.links.linkedin}>{users.links.linkedin}</a>
              : <p className="text-2xl text-grey">Not Submitted</p>}
              <Extlink />
            </div>

            <div className={`profile_links flex items-center px-4 py-3 rounded-full justify-between text-2xl text-white ${!users.links.hackerrank ? 'bg-blue/30' : 'bg-blue hover:bg-blue/80'}`}>
              <Hackerrank/>
              {users.links.hackerrank ? <a target="_blank" rel="noreferrer" className="overflow-hidden line-clamp-1 text-ellipsis underline" href={users.links.hackerrank}>{users.links.hackerrank}</a>
              : <p className="text-2xl text-grey">Not Submitted</p>}
              <Extlink />
            </div>

            <div className={`profile_links flex items-center px-4 py-3 rounded-full justify-between text-2xl text-white ${!users.links.dribble ? 'bg-blue/30' : 'bg-blue hover:bg-blue/80'}`}>
              <Dribbble/>
              {users.links.dribble ? <a target="_blank" rel="noreferrer" className="overflow-hidden line-clamp-1 text-ellipsis underline" href={users.links.dribble}>{users.links.dribble}</a>
              : <p className="text-2xl text-grey">Not Submitted</p>}
              <Extlink />
            </div>

            <div className={`profile_links flex items-center px-4 py-3 rounded-full justify-between text-2xl text-white ${!users.links.behance ? 'bg-blue/30' : 'bg-blue hover:bg-blue/80'}`}>
              <Behance/>
              {users.links.behance ? <a target="_blank" rel="noreferrer" className="overflow-hidden line-clamp-1 text-ellipsis underline" href={users.links.behance}>{users.links.behance}</a>
              : <p className="text-2xl text-grey">Not Submitted</p>}
              <Extlink />
            </div>

            <div className={`profile_links flex items-center px-4 py-3 rounded-full justify-between text-2xl text-white ${!users.links.custom ? 'bg-blue/30' : 'bg-blue hover:bg-blue/80'}`}>
              <Customlink/>
              {users.links.custom ? <a target="_blank" rel="noreferrer" className="overflow-hidden line-clamp-1 text-ellipsis underline" href={users.links.custom}>{users.links.custom}</a>
              : <p className="text-2xl text-grey">Not Submitted</p>}
              <Extlink />
            </div>
          </div>
